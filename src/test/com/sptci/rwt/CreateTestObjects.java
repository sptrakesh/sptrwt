 ...
 * @version $Id: CreateTestObjects.java 3559 2007-10-02 22:43:50Z rakesh $
 */
public class CreateTestObjects extends TestCase
{
  ...
  protected ConnectionParameters getParameters()
  {
    ...
    return new ConnectionParameters( user, password, host,
        Integer.parseInt( port ), name, type, url, driver );
  }
  ...
  protected void createTable() throws Exception
  {
    String ddl = "create table $TABLENAME$ ( \n" +
      ...
      "constraint fk_$TABLENAME$_$REFTABLENAME$ " + 
        "foreign key (name) references $TABLENAME$ " + 
    ...
    System.out.format( "Created test tables: %s, %s and view: %s%n",
        tableName, refTableName, viewName );

    assertEquals( "Ensuring ddlCount results returned",
        rows.size(), ddlCount );
    assertEquals( "Ensuring single row results returned",
        rows.get( 0 ).getRows().size(), 1 );
    assertEquals( "Ensuring only one column in row results",
